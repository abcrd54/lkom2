...
async function handleConnect(request: Request) {
  try {
    ...
    const authorizationUrl = await createOAuthAuthorizationUrl("microsoft");
    ...
    url.searchParams.set("oauth_provider", "microsoft");
    url.searchParams.set("oauth_status", "error");
    url.searchParams.set(
      "oauth_message",
      error instanceof Error ? error.message : "Failed to start Microsoft OAuth."
    );
